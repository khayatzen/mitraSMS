...
class Messages extends MY_Controller {
		...
		if ($type=='forward') 
		{
                        ...
                        switch ($source)
			{
				...
				break;
                    ...
                    $data['id_folder'] = $this->input->post('param2');
                    $data['id_inbox'] = $this->input->post('param3');
                }
	...
	function compose_process()
	{
		...
		switch($this->input->post('sendoption')) 
		{
			...
			$dest[] = $this->input->post('reply_value');
                        $data['id_folder'] = $this->input->post('id_folder');
                        $data['id_inbox'] = $this->input->post('id_inbox');
			break;
		...
		$data['uid'] = $this->session->userdata('id_user');
                // Cek apakah mode SMS broadcast
                if($this->input->post('broadcast_mode'))$data['is_broadcast']=1;
                else $data['is_broadcast']=0;
                if($this->input->post('is_forward'))$data['is_forward']=1;                
                //$data['is_broadcast'] = ($this->input->post('broadcast_mode')) ? '0' : '1';                
	...
	{		
		...
		if(!in_array($type, $valid_type)) die('format URL Salah');
		...
		else 
		{
			...
			$param['offset'] = $this->uri->segment(4,0);
                        //$param = array('option' => 'paginate', 'limit' => $config['per_page'], 'offset' => substr($this->uri->segment(3,0),10));
			$data['messages'] = $this->Message_model->get_conversation($param);
	...
	{
                if($id_folder)$data['title'] = $this->Kalkun_model->get_folders('name',$id_folder)->row('name');
		// validate url
		...
		if(isset($id_user) && !empty($id_user)) $param['uid'] = $this->session->userdata('id_user');
		if(is_ajax())
		{
			$param['type'] = $type;
	 ...
	 * @access	public
	 */		
	function conversation($source=NULL, $type=NULL, $number=NULL, $id_folder=NULL, $callback=NULL)
	{
	...
	function _multiple_modem_select($phone_number, $date) 
	{
        ...
        /*
         * Publish inbox message
         */
        function publish_message(){
            $id_message = (int)$this->input->post('id_message');
            return $this->Message_model->set_message_published($id_message);
        }
        /*
         * Spam
         * -----------
        ...
        function mark_spam(){
            ...
            $this->Message_model->delete_messages($param);
            $this->Phonebook_model->mark_spam($phone_number);
        }
}	
