...
else 
{
	...
	<div class="message_container <?php if($type == 'inbox' && $tmp->published=='false') echo 'unpublished';?>">
		<div class="message_header" style="color: #444; height: 20px; overflow: hidden">
		<input type="checkbox" id="<?php echo $number;?>" id_message="<?=$tmp->ID?>" class="select_conversation nicecheckbox" value="<?php echo $number;?>" style="border: none;" />
		...
		&nbsp;<?php //echo $message_date;?>&nbsp;<?php echo $senderName;?>
		<?php 
			if($folder=='folder'):
		...
		<span class="message_preview" 
                        <?php  
                        if($type == 'inbox' && $tmp->readed=='false') echo "style=\"font-weight: bold\"";
