...
var MultimediaGallery = {
	...
	*/
	start					: function () {
		$.get('berkas/gettotalfiles', function(data) {
	...
	*/
	display					: function () {
		...
		$.get('berkas/display' , function(data) {
			...
			for(var i=0; i<res_length; ++i){
				...
				}).attr('src', 'http://localhost/gis-jalan/' + elem_thumb)
		...
		*/
		...
		switch(item_type){
			case 'photo':
				...
				}).attr('src', 'http://localhost/gis-jalan/' + item_sources);
				break;
			case 'audio':
			...
			case 'video':
				...
				//for(var i = 0; i < sources_length; ++i){
				//	var theSource 	= item_sources[i].source;
                                        var theSource   = 'http://localhost/gis-jalan/' +  item_sources;
